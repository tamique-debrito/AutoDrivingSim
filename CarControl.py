...
from DataClasses import ControlOutput, ObstaclesInfo
...
class CarControl:
    ...
    MAX_ANGLE = 0.4
    COMMAND_CHANGE_INTERVAL_LOWER: int = 10
    COMMAND_CHANGE_INTERVAL_UPPER: int = 10
    def __init__(self, world: World):
        ...
        self.next_command_change_time = random.uniform(self.COMMAND_CHANGE_INTERVAL_LOWER, self.COMMAND_CHANGE_INTERVAL_UPPER)
    
    def step(self, obstacles_info: ObstaclesInfo, time_delta: float, current_time: float, car_info: CarInfo) -> ControlOutput:
        """
        ...
        if current_time >= self.next_command_change_time:
            self.current_command_state = random.choice(list(CommandState))
            self.next_command_change_time = current_time + random.uniform(self.COMMAND_CHANGE_INTERVAL_LOWER, self.COMMAND_CHANGE_INTERVAL_UPPER)
